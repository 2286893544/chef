<template>
  <div class="rich-text-editor">
    <div class="editor-header">
      <el-button type="info" @click="goBack" style="">返回</el-button>
...
let richText = sessionStorage.getItem('richText')
  ? JSON.parse(sessionStorage.getItem('richText') as string)
  : ''
...
async function uploadImage(file: File) {
  ...
  axios
    ...
    .then((response) => {
      ...
      if (range) {
        // 默认居中
        const imgTag = `<img src="${imageUrl}" alt="Uploaded Image" style="max-width: 50%; height: auto;" margin: auto; />`
...
function modifyResume(richText: string) {
  service
    .put('/changeRichTextMsg', { id: uId, content: richText })
    .then((res: any) => {
      if (res.code == 200) {
...
function inlineStyles(htmlContent: string): string {
  ...
  elements.forEach((element: any) => {
    const alignClass: any = Array.from(
      element.classList
    ).find((className: any) => className.startsWith('ql-align-'))
    if (alignClass) {
